  ...
  return (
    <>
      <div className={classes.table_wrapper}>
                  ...
                  <Tr>
                    ...
                    <Td>
                      {item?.approval_levels.length === 0
                        ? "Dop"
                        : item?.approval_levels.length === 1 &&
                          item?.DOPApprovalStatus === "rejected"
                        ? "Dop"
                        : item?.approval_levels.length === 1 &&
                          item?.DOPApprovalStatus === "approved"
                        ? "Accountant"
                        : item?.approval_levels.length === 2 &&
                          item?.accountantApprovalStatus === "rejected"
                        ? "Accountant"
                        : item?.approval_levels.length === 2 &&
                          item?.accountantApprovalStatus === "approved"
                        ? "Ceo"
                        : item?.approval_levels.length === 3 &&
                          item?.CEOApprovalStatus === "rejected"
                        ? "Ceo"
                        : item?.approval_levels.length === 3 &&
                          item?.CEOApprovalStatus === "approved"
                        ? "Procurement Officer"
                        : "Procurement Officer"}
                    </Td>
                    <Td>
                      {item?.approval_levels.length === 0
                        ...
                        : item?.approval_levels.length === 4 &&
                          item?.procurementApprovalStatus === "rejected"
                        ? item?.procurementApprovalStatus
                        : item?.procurementApprovalStatus}
                    </Td>
        ...
        {data && data.length < 1 && (
          <div className={classes.no_records_wrapper}>
