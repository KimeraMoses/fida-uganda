...
const TravelOrder = () => {
    ...
    const {data: pendingDopTravelOrders, isLoading:dopPending} = usePendingDopTravelOrders();
    const {data: approvedDopTravelOrders, isLoading:dopApproved} = useApprovedDopTravelOrders();
    const {data: rejectedDopTravelOrders, isLoading:dopRejected} = useRejectedDopTravelOrders();

    const {data: pendingAccountantTravelOrders, isLoading:accountsPending} = usePendingAccountantTravelOrders();
    const {data: approvedAccountantTravelOrders, isLoading:accountsApproved} = useApprovedAccountantTravelOrders();
    const {data: rejectedAccountantTravelOrders, isLoading:accountsRejected} = useRejectedAccountantTravelOrders();

    const {data: pendingFleetManagerTravelOrders, isLoading:fleetPending} = usePendingFleetManagerTravelOrders();
    const {data: approvedFleetManagerTravelOrders, isLoading:fleetApproved} = useApprovedFleetManagerTravelOrders();
    const {data: rejectedFleetManagerTravelOrders, isLoading:fleetRejected} = useRejectedFleetManagerTravelOrders();
...
//set data for pending DOP table
    ...
    useEffect(() => {
        ...
        if (pendingAccountantTravelOrders?.TravelOrders?.length) {
            const dataToSet = pendingAccountantTravelOrders?.TravelOrders?.map((b) => {
                return {
                    ...b,
                    full_name: b?.createdBy?.full_name
                        ? b.createdBy.full_name
    ...
    useEffect(() => {
        ...
        if (approvedAccountantTravelOrders?.TravelOrders?.length) {
            const dataToSet = approvedAccountantTravelOrders?.TravelOrders?.map((b) => {
                return {
                    ...b,
                    full_name: b?.createdBy?.full_name
                        ? b.createdBy.full_name
    ...
    useEffect(() => {
        ...
        if (rejectedAccountantTravelOrders?.TravelOrders?.length) {
            const dataToSet = rejectedAccountantTravelOrders?.TravelOrders?.map((b) => {
                return {
                    ...b,
                    full_name: b?.createdBy?.full_name
                        ? b.createdBy.full_name
...
//set table data for pending Fleet manager table
    useEffect(() => {
        ...
        if (pendingFleetManagerTravelOrders?.TravelOrders?.length) {
            const dataToSet = pendingFleetManagerTravelOrders?.TravelOrders?.map((b) => {
                return {
                    ...b,
                    full_name: b?.createdBy?.full_name
                        ? b.createdBy.full_name
    ...
    useEffect(() => {
        ...
        if (approvedFleetManagerTravelOrders?.TravelOrders?.length) {
            const dataToSet = approvedFleetManagerTravelOrders?.TravelOrders?.map((b) => {
                return {
                    ...b,
                    full_name: b?.createdBy?.full_name
                        ? b.createdBy.full_name
    ...
    useEffect(() => {
        ...
        if (rejectedFleetManagerTravelOrders?.TravelOrders?.length) {
            const dataToSet = rejectedFleetManagerTravelOrders?.TravelOrders?.map((b) => {
                return {
                    ...b,
                    full_name: b?.createdBy?.full_name
                        ? b.createdBy.full_name
